...
def get_data_slice(first=1000):
    ...
    train_tf_file = tf.keras.utils.get_file("train_tf_processed.tfrecord", dataset_url + "train_tf_processed.tfrecord")
    validate_tf_file = tf.keras.utils.get_file(
        "validate_tf_processed.tfrecord", dataset_url + "validate_tf_processed.tfrecord"
    )
...
def build_and_train_model():
    ...
    return tfma.export.export_eval_savedmodel(
        estimator=classifier, export_dir_base=MODEL_PATH, eval_input_receiver_fn=eval_input_receiver_fn
    )
