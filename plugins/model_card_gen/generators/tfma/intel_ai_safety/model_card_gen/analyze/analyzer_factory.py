...
from intel_ai_safety.model_card_gen.analyze import TFAnalyzer, PTAnalyzer, DFAnalyzer
from intel_ai_safety.model_card_gen.datasets import TensorflowDataset, PytorchDataset


def get_analyzers(
    model_path: Optional[Text] = "", eval_config: Union[tfma.EvalConfig, Text] = None, datasets: DatasetType = None
):
    ...
    if all(isinstance(ds, TensorflowDataset) for ds in datasets.values()):
        analyzers = (TFAnalyzer(model_path, dataset, eval_config) for dataset in datasets.values())
    elif all(isinstance(ds, pd.DataFrame) for ds in datasets.values()):
        analyzers = (DFAnalyzer(dataset, eval_config) for dataset in datasets.values())
    elif all(isinstance(ds, PytorchDataset) for ds in datasets.values()):
        analyzers = (PTAnalyzer(model_path, dataset, eval_config) for dataset in datasets.values())
    else:
        analyzers = ()
    return analyzers
...
def get_analysis(
    model_path: Optional[Text] = "", eval_config: Union[tfma.EvalConfig, Text] = None, datasets: Dict = {}
):
