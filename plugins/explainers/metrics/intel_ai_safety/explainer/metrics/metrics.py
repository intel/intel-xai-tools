...
class ConfusionMatrix:
    ...
    def __init__(self, groundtruth, predictions, labels):
        ...
        self.df = pd.DataFrame(self.arr, index=[i for i in self.labels], columns=[i for i in self.labels])
    ...
    def visualize(self):
        ...
        s = sn.heatmap(self.df, annot=True, cmap=sn.color_palette("Blues", as_cmap=True), linewidths=2, cbar=False)
