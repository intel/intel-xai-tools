...
from intel_ai_safety.explainer.attributions.plots import shap_waterwall_plot
from intel_ai_safety.explainer.attributions.widgets import ShapUI
...
def test_shap_ui():
    kwargs = dict(
        df=pd.DataFrame({"Feature 1": np.random.rand(10), "Feature 2": np.random.rand(10)}),
